...
async function main() {
  console.log("== Correctly Adding Issues and Setting Status on GitHub Project Board (v2) ==");
  ...
  for (let issueNum = 1; issueNum <= 28; issueNum++) {
    ...
    if (issueRes.status === 200 && issueRes.data?.node_id) {
      ...
      // 1. Add item using addProjectV2ItemById
      ...
      if (itemId) {
        ...
        `);
        console.log(`✅ Issue #${issueNum} linked to Project Board & Status set to 'Done': item ID ${itemId}`);
      } else {
        console.error(`Error adding Issue #${issueNum}:`, JSON.stringify(addRes));
      }
    }
  }

  console.log("== Finished Updating All 28 Project Items into 'Done' Board Column! ==");
}
